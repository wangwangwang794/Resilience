...
global Settings dist nos line_out roa sens fault_time fault_dur tcr 
global max_time flt_bus rad xx0 tvalue xy0 fltno res_lambda cpf l_event done ld_par
global vstack qstack istack qids
global buscon linecon pqcon swcon syncon pvcon synid ref_bus
 ...
 case 'pf'   % solve power flow
  ...
  if tcr == 1
      ...
      if length(line_out) == 2
          ...
          Line.con(key,9) = dist(nos)*Line.con(key,9);
          Line.con(key,10) = dist(nos)*Line.con(key,10);

          Line.con(key + 1,9) = (1 - dist(nos))*Line.con(key + 1,9);
          Line.con(key + 1,10) = (1 - dist(nos))*Line.con(key + 1,10);
      else
          id = find(Bus.con(:,1) == line_out);
          Fault.con = [line_out	  100   Bus.con(id,2)   60	  fault_time	(fault_time + fault_dur)  0	0];
      end
  end
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  if Settings.init
