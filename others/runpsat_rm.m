...
global Settings dist line_out fault_time fault_dur 
global buscon linecon pqcon swcon syncon pvcon maxbus flt_type
...
switch routine
 ...
 case 'pf'   % solve power flow
  ...
  %%%%%%% Added for Relay Margin analysis%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  Bus.con = buscon;
  Line.con = linecon;
  PQ.con = pqcon;
  SW.con = swcon;
  Syn.con = syncon;
  PV.con = pvcon;
  if Line.con(end,9)<0.025
      Line.con(end,9) = 0.025;
  end
    
  if strcmp(flt_type,'line')||strcmp(flt_type,'trafo')||strcmp(flt_type,'gen')
      if strcmp(flt_type,'gen')
           key = find((Line.con(:,1) == line_out)|(Line.con(:,2) == line_out));           
           line_out = Line.con(key,1:2);
           dist = 0.5;
      else
           key = find((Line.con(:,1) == line_out(1))&(Line.con(:,2) == line_out(2)));
      end
      maxbus = Bus.con(end,1);
      Bus.con = [Bus.con;Bus.con(end,:)];
      Bus.con(end,1) = maxbus + 1;
      Bus.con(end,2) = Line.con(key,4);
      Bus.con(end,3) = 1;
      if ~strcmp(flt_type,'gen')
        Fault.con = [maxbus + 1	Line.con(key,3:5)	fault_time	(fault_time + fault_dur)  0	0];
      end
      temp = Line.con;
      temp1 = temp;
      jj = 1;
      for ii = 1:length(Line.con(:,1))
          if ii == key
              temp1(ii,:) = temp(jj,:);
              temp1(ii+1,:) = temp(jj,:);
              jj = jj + 1;
      ...
      Line.con = temp1;
      Line.con(key,2) = maxbus + 1;
      Line.con(key + 1,1) = maxbus + 1;      

      Line.con(key,9) = dist*Line.con(key,9);
      Line.con(key,10) = dist*Line.con(key,10);

      Line.con(key + 1,9) = (1 - dist)*Line.con(key + 1,9);
      Line.con(key + 1,10) = (1 - dist)*Line.con(key + 1,10);
      
      if ~(Line.con(key,7)==0)
          Line.con(key,7) = sqrt(Line.con(key,7));
          Line.con(key+1,7) = Line.con(key,7);
          Line.con(key+1,4) = round(Line.con(key+1,4)/Line.con(key,7),1);
          id_tmp = find(Bus.con(:,1)== maxbus);
          Bus.con(id_tmp+1,2) = Line.con(key+1,4);
          if ~strcmp(flt_type,'gen')
            Fault.con(3) = Line.con(key+1,4);
          end
      end

      Breaker.con = [key    maxbus + 1	Line.con(key,3:5)	fault_time	(fault_time + fault_dur)  10000;...
                     key+1  maxbus + 1	Line.con(key,3:5)	fault_time	(fault_time + fault_dur)  10000];
  end
  
  if strcmp(flt_type,'bus')
      key = find((Line.con(:,1)==line_out)|(Line.con(:,2)==line_out));
      Fault.con = [line_out	Line.con(key(1),3:5)	fault_time	(fault_time + fault_dur)  0	0];
      temp2 = [line_out	Line.con(key(1),3:5)	fault_time	(fault_time + fault_dur)  10000];
      temp2 = repmat(temp2, length(key), 1);
      Breaker.con = [key temp2];      
  end
